...
## Modify repo column
df_community["repo"] = df_community["repo"].replace(
    "^", "https://github.com/", regex=True
)
...
df_seatable_only = df_community.merge(
    df_seatable.drop_duplicates(),
    on=["id"],
    how="right",
    indicator=True,
    suffixes=("_community", "_seatable"),
)
...
df_diff = pd.concat([df_seatable, df_community]).drop_duplicates(
    subset=["name", "description", "repo"], keep=False
)
